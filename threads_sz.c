...
{
   int cur_time = 0,i = 0;
   static int oldPassedNum = 0;
   time_t tt = 0;
   struct tm *local = NULL;
   for( i = 0;i<TAG_OLD_PASSED_MAX;i++ )
   {
       if( !strncmp(opt_info->TID,_old_passed_array[i].TID,TID_LEN) ) 
       {
            break;
       }
   }//end for for( i = 0;i<.......
   ...
   if( TAG_OLD_PASSED_MAX == i ){
        if(oldPassedNum >= TAG_OLD_PASSED_MAX)
        {
            oldPassedNum = 0;
        }
        strncpy(_old_passed_array[oldPassedNum].TID,opt_info->TID,TID_LEN);
        _old_passed_array[oldPassedNum].time = cur_time;
        //printf("first come here ,can enter!!!\n");
        oldPassedNum++;
        return true;
   }else if( i< TAG_OLD_PASSED_MAX ){
        if( cur_time < _old_passed_array[i].time)
            cur_time += 24 * 3600;
        if( cur_time - _old_passed_array[i].time > 6){
         // printf("great 6 second,can enter!!!\n");
           _old_passed_array[i].time = cur_time;
           return true;
        }else{
          //printf("less 6 second,can't enter!!!\n");
           return false;
        }
   }//end for if( TAG_OLD_PASSED_MAX == i....
}
...
int GetTagsAndDeal(int *whitchInduction)
{
        ...
        if(strcmp(tag->tid,""))
	{
           ...
           str_assign_value(tag->tid,g_tags_array[i].tid,TID_LEN);   
